...
void formula(vec2 z, float c) {
	...
	vec3 col=normalize(.1+texture(diffuse,vec2(minit*.1)).rgb);
	color+=col*(.4+mod(minit/9.-time*10.+ot2*2.,1.)*1.6);

	color+=vec3(1.,.7,.3)*circ*(10.-minit)*3.*smoothstep(0.,.5,.15+0-.5);
...
//	zoom*=1.+sin(time*3.758123)*.8;
	...
	outColor = vec4(colo,1.0);

    float rrr = outColor.r;
    float ggg = outColor.g;
