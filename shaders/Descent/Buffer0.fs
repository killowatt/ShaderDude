...
void main()
{
   ...
   vec2 uv = gl_FragCoord.xy/resolution.xy;
    vec2 pu = vec2(uv.x, uv.y * (resolution.y/resolution.x));
    ...
    vec3 old = texture(bufferOne, uv ).rgb;
    old += 1.0/255. * rand(uv+vec2(t)) - 0.1/255.0;
    vec2 ncoord = pu + vec2(sin(time*199.), sin(time*238.));
